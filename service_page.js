...
async function getDATA() {
  const data = await fetch("../milk-body/data.json");
  return data.json();
}
...
const createMarkupMainPrice = `<tr>
      ...
      <td class="service_price_table_row old_price">${
        serviceMainPrice.price_old
      }</td>
      <td class="service_price_table_row new_price">${
        serviceMainPrice.price
      }</td>
    </tr>
`;
...
<td colspan="4" class="service_price_table_row service_price-notes">При замовленні послуги на двох  -25%</td>
</tr>`;
